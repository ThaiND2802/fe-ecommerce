...
const DataFilter = () => {
  ...
  const filterFields = useMemo(() => {
    const allFields = {
      ...
      [EColumnKey.OrderId]: { type: 'order' },
      [EColumnKey.ProductId]: { type: 'product' },
      [EColumnKey.Coefficient0]: { type: 'numberRange' },
      [EColumnKey.Coefficient1]: { type: 'numberRange' },
      [EColumnKey.Coefficient2]: { type: 'numberRange' },
      [EColumnKey.Coefficient3]: { type: 'numberRange' },
      [EColumnKey.Coefficient4]: { type: 'numberRange' },
      [EColumnKey.Coefficient5]: { type: 'numberRange' },
      [EColumnKey.Quantity]: { type: 'numberRange' },
      [EColumnKey.UnitPrice]: { type: 'numberRange' },
      [EColumnKey.Total]: { type: 'numberRange' },
    } as const
      ...
      .map(([key, value]) => {
        switch (value.type) {
          ...
          case 'order':
            return {
              key,
              Element: <OrderSelector allowClear placeholder={`${t.text.input}`} />,
            }
        }
      })
