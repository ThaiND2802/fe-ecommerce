...
import { InventoryItem } from 'src/entities/management/inventory/types'
...
export type ITableData = InventoryItem
...
export const useTableData = () => {
  ...
  const { data: inventoryList, isLoading, refetch } = useQuery({
  ...
  useEffect(() => {
    if (inventoryList) {
      setDatasource(inventoryList.data)
      setTableParams({
        ...tableParams,
        pagination: {
          ...tableParams.pagination,
          total: inventoryList.pagination?.total_rows,
        },
      })
    }
  }, [inventoryList])
