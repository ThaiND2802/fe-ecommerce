...
export type ITableData = ProductItem
...
export const useTableData = () => {
  ...
  const { data: productList, isLoading, refetch } = useQuery({
    ...productQueries.list({
  ...
  useEffect(() => {
    if (productList) {
      setDatasource(productList.data)
      setTableParams({
        ...tableParams,
        pagination: {
          ...tableParams.pagination,
          total: productList.pagination?.total_rows,
        },
      })
    }
  }, [productList])
